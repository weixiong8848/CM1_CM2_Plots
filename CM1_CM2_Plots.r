...
#2. Origanize you acmo.csv as the following rules
#   2.1. Save all acmo.csv of APSIM in another foder named as 1, and acmo.csv of DSSAT in a folder named as 2.
#   2.2. Trim the name of acmo.csv to a four digit name as the scenario, such as change "ACMO-KURNOOL-0-0XFX-0-0-APSIM.csv" to "0XFX.csv".
#3. Execut the script, plots are saved in a pdf file with the name that you defined in pngoutput.
#############################################################################
...
plots<-function(title,plottype,plotformat,plotvariable,yieldorratio,inputdir,group,count,pngoutput,output_acmo){
    ...
    #temp<-NA
    #title<-paste(title, " of KURNOOL",sep="")
...
plottype<-"b"    # Type of the plots, b: Box/whisker, c: CDF, cumulative distribution
plotformat<-"pdf" # No need to change
plotvariable<-"HWAH_S" # Plot variables, no need to change
yieldorratio<-"r" #Ploting absolute yield (y) or change ratio (r)
inputdir<-"/Users/weixiong/Development/face-it/RIA/ria_standardplots/1.0.5_either input zip or csv/3"# You directory that put you results and script.
...
pngoutput<-"Ratio.pdf" # Name of the plot output file, Ratio.paf or Yield.pdf
